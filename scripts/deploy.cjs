...
async function main() {
  ...
  const kkdToken = await KKDToken.deploy(deployerAddr);
  ...
  const puerToken = await RWAToken.deploy("Vintage Pu'er 2005", "PUER", hre.ethers.parseEther("100000"), deployerAddr);
  ...
  const agedToken = await RWAToken.deploy("Imperial Aged Tea", "AGED", hre.ethers.parseEther("10000"), deployerAddr);
  ...
  const ceraToken = await RWAToken.deploy("Yixing Ceramics Token", "CERA", hre.ethers.parseEther("500000"), deployerAddr);
  ...
  const nft = await KKIKDANft.deploy(deployerAddr);
  ...
  const marketplace = await Marketplace.deploy(deployerAddr);
  ...
  const staking = await Staking.deploy(kkdAddress, deployerAddr);
  ...
  console.log("========================================");
  console.log(`\nOwner:         ${deployerAddr}`);
  console.log("\n📋 Copy these addresses to src/lib/web3/contracts.ts");
}
